...
CHAIN_CONFIG = {
    **ANCHOR_CONFIG,
    "mechababs": {**ANCHOR_CONFIG["mechababs"],
                  "depends_on": "SimBIDS-0.0.3+anchor"},
    "input_datasets": {"SimBIDS-0.0.3+anchor": {"is_zipped": True}},
}

CLUSTER_CONFIG = {"script_preamble": "activate {{MECHABABS_VENV}}\n",
...
def test_a_named_sourcedata_carries_its_id_into_the_derivative():
    ...
    assert scaffold.derivative_name("sourcedata/ds000001", ANCHOR) == \
        "SimBIDS-0.0.3+anchor+ds000001"
    assert scaffold.derivative_path("sourcedata/ds000001", ANCHOR) == \
...
def test_a_container_url_is_passed_through_but_a_relative_path_resolves(tmp_path):
    ...
    assert scaffold.resolve_container_ds(
        tmp_path, {"source": "https://github.com/ReproNim/containers.git"}) == \
        "https://github.com/ReproNim/containers.git"
    assert scaffold.resolve_container_ds(tmp_path, {"source": "../containers"}) == \
        str((tmp_path / "../containers").resolve())
    assert scaffold.resolve_container_ds(tmp_path, {"source": "/abs/containers"}) == \
        "/abs/containers"
...
def test_the_source_url_comes_from_the_studys_own_gitmodules(tmp_path):
    ...
    assert scaffold.source_dataset_url(tmp_path, SOURCEDATA) == \
...
def study(tmp_path):
    ...
    campaign_mod.config_path(study, LABEL).write_text(yaml.safe_dump({
        "label": LABEL, "apps": [ANCHOR, CHAIN],
        "cluster": "clusters/test.yaml", "limit": 1,
    }))
    ...
    campaign_mod.write_state(study, LABEL, [
        {"source_dataset": SOURCEDATA, "app_config": ANCHOR, "depends_on": "",
         **identity},
        {"source_dataset": SOURCEDATA, "app_config": CHAIN, "depends_on": ANCHOR,
         **identity},
    ])
    return study
...
def _row(study, app_config):
    return next(r for r in campaign_mod.read_state(study, LABEL)
...
def test_the_composed_config_reaches_babs_and_carries_all_three_axes(study,
                                                                    babs_calls):
    scaffold.scaffold(study, LABEL, SOURCEDATA, ANCHOR)
    ...
    assert config["input_datasets"]["BIDS"]["origin_url"] == \
        "https://example.org/ds999999.git"                         # source axis
...
def test_a_dependent_cell_is_refused_while_its_producer_is_unmerged(study,
                                                                    babs_calls):
...
def test_a_merged_producer_wires_its_output_ria_into_the_dependent(study,
                                                                  babs_calls):
    ...
    origin = babs_calls[0]["config"]["input_datasets"]["SimBIDS-0.0.3+anchor"]["origin_url"]
