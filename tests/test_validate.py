...
# --- clone_ref: what `git clone --branch` can actually re-clone ---------------------
# The riskiest logic here (a pin that resolves to something unclonable fails far from
# its cause), so it gets real git repos rather than mocks.
...
def test_clone_ref_falls_back_to_the_tag_when_detached(tmp_path):
    ...
    "HEAD" — which `git clone --branch` cannot resolve."""
    import subprocess
